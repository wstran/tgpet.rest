    ...
    router.post('/store/pet', Middleware, async (req, res) => {
        ...
        try {
            ...
            const [update_user_result, insert_pet_result, insert_log_result] = await Promise.all([
                userCollection.updateOne({ tele_id: tele_user.tele_id, ...$USER_FILTER }, { ...$USER_UPDATE, $inc: { ...$USER_UPDATE.$inc, 'totals.spent': total_cost, 'totals.spent_pet': total_cost } }, { session }),
                petCollection.insertOne({ tele_id: tele_user.tele_id, type: pet_name, level: pet_level, mana: now_date, max_mana: pet.max_mana, accumulate_total_cost: total_cost }, { session }),
                logCollection.insertOne({ log_type: 'store/pet', tele_id: tele_user.tele_id, pet_name, total_cost, tgp_balance, tgpet_balance, total_balance, created_at: new Date(), ...$LOG_INSERT }, { session })
            ]);

            if (
                update_user_result.modifiedCount > 0 &&
                insert_pet_result.acknowledged === true &&
