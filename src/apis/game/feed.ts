    ...
    router.post('/game/feed', Middleware, async (req, res) => {
        ...
        try {
            ...
            const [user, pet] = await Promise.all([
                ...
                petCollection.findOne({ _id: pet_object_id }, { projection: { _id: 1, mana: 1, max_mana: 1, farm_at: 1, accumulate_total_cost: 1 }, session }) as Promise<Pet>
            ]);

            if (user === null) {
                return res.status(404).json({ message: 'Not found.', status: 'NOT_ENOUGH_FOOD' });
            };
            ...
            const max_mana = date_timestamp + (pet.max_mana * 28800000);

            const new_mana = mana_timestamp + (config_game_items.items[item_name].mana * 28800000);

            const mana = new_mana > max_mana ? max_mana : new_mana;
