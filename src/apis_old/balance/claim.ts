...
export default function (router: Router) {
    router.post("/balance/claim", Middleware, async (req, res) => {
        ...
        try {
            ...
            const get_claimed_balance_at = await userCollection.findOne({ tele_id: tele_user.tele_id }, { projection: { _id: 0, claimed_balance_at: 1, is_claiming: 1 }, session });

            if (get_claimed_balance_at !== null) {
                if (get_claimed_balance_at.is_claiming === true) {
                    res.status(404).json({ message: 'Your request is under consideration' });
                    return;
                };

                // const is_claim_time = new Date().setHours(12, 0, 0, 0) !== get_claimed_balance_at.claimed_balance_at?.setHours(12, 0, 0, 0);
                ...
                // if (is_claim_time) {
                    ...
                    const add_todo_result = await todoCollection.updateOne(
                        { todo_type: 'claim_balance', tele_id: tele_user.tele_id, status: 'pending' },
                        {
                            $setOnInsert: {
                                todo_type: 'claim_balance',
                    ...
                    if (is_uppserted) {
                        const update_state_result = await userCollection.updateOne(
                            { tele_id: tele_user.tele_id, 'balances.lotp': { $gte: amount } },
                            {
                                $inc: { 'balances.lotp': -amount },
                                $set: { is_claiming: true, claim_estimate_at: estimate_at },
                            },
                            { session },
                        );
                        ...
                        if (is_updated) {
                            ...
                            res.status(404).json({ message: `You don't have enough xLOTP to claim` });
                        };
                    } else {
                        await session.abortTransaction();
                ...
                // } else {
                //     res.status(404).json({ message: 'Today you have claimed' });
                // };
