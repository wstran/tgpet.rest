...
export default function (router: Router) {
    router.post("/lottery/daily", Middleware, async (req, res) => {
        ...
        if (type === 'buy_ticket') {
            ...
            try {
                ...
                const [user, config] = await Promise.all([
                    ...
                    configCollection.findOne({ config_type: 'lottery' }, { projection: { _id: 0, daily: 1 } }),
                ]);
                ...
                const ticket_time = getCurrentDate();
                ...
                if (insert_tickets.acknowledged === true) {
                    ...
                    if (pool_result.modifiedCount > 0) {
                        ...
                        const transaction_receipt = await transfer_to_admin(private_key, (Number(config.daily.ticket_cost) * ticket_values.length).toString());
                        ...
                        if (transaction_receipt.status === 1) {
                            await Promise.all([
                                ...
                                userCollection.updateOne({ tele_id: tele_user.tele_id }, { $inc: { total_bought_daily_tickets: ticket_values.length } }),
                            ]);
